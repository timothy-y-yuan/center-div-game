...
export default function HintPopup({ isOpen, hint, onClose, buttonRef, onRevealAnswer, isCompleted, isFailed, solutionCSS, explanation }: HintPopupProps) {
  ...
  return createPortal(
    <div ref={popupRef} style={popupStyle} className="w-80 animate-in slide-in-from-top-2 duration-200">
      {/* Arrow */}
      ...
      <div className={`relative rounded-xl shadow-xl overflow-hidden ${
        ...
        {/* Header */}
        <div className={`p-4 ${(isCompleted || isFailed)
          ? (isCompleted ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' : 'bg-gradient-to-r from-red-400 to-red-500')
          : 'bg-gradient-to-r from-amber-400 to-orange-500'
        }`}>
          <div className="flex items-center gap-3">
            <span className="text-xl">{isCompleted ? '🤓' : isFailed ? '😭' : '💡'}</span>
            <h3 className="font-bold text-white">{(isCompleted || isFailed) ? 'Solution' : 'Hint'}</h3>
          </div>
        </div>
        ...
        <div className="p-4 pb-2">
          {(isCompleted || isFailed) ? (
            <div>
              ...
              <pre className={`text-xs p-3 rounded-lg overflow-x-auto mb-3 ${
                actualTheme === 'dark'
