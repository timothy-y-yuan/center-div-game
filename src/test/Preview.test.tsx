...
describe('Preview Component', () => {
  const defaultProps = {
    content: '<!DOCTYPE html><html><head></head><body><div>Test</div></body></html>',
    isCompleted: false,
    currentLevel: 0,
    totalLevels: 10,
    onNextLevel: vi.fn()
  ...
  it('should update iframe content when content prop changes', () => {
    ...
    const { rerender } = render(<Preview {...defaultProps} content={initialContent} />)
  ...
  it('should have correct iframe styling and attributes', () => {
    ...
    const iframe = document.getElementById('preview')
    expect(iframe).toHaveClass('w-full', 'h-full', 'border-0', 'rounded-xl', 'shadow-lg', 'bg-white')
  })
  ...
  it('should handle numbers in content', () => {
    render(<Preview {...defaultProps} content="<html><body>10</body></html>" />)
  ...
  it('should have proper container styling', () => {
    ...
    const previewContainer = container.querySelector('.h-full.flex.flex-col.glass')
    expect(previewContainer).toBeInTheDocument()
    expect(previewContainer).toHaveClass('h-full', 'flex', 'flex-col', 'glass', 'rounded-2xl', 'overflow-hidden', 'ml-2')
  })
  ...
  it('should not show next level button on last level', () => {
    render(<Preview {...defaultProps} isCompleted={true} currentLevel={9} totalLevels={10} />)
