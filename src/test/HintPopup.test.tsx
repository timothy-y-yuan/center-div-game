...
describe('HintPopup Component', () => {
  ...
  it('should show solution and explanation for completed levels', () => {
    ...
    expect(screen.getByText('.container { margin: 0 auto; }')).toBeInTheDocument()
    expect(screen.getByText('💡 Why it works:')).toBeInTheDocument()
    expect(screen.getByText('This centers the element horizontally using auto margins.')).toBeInTheDocument()
  })
  ...
  it('should handle long explanation text properly', () => {
    const propsWithLongExplanation = {
      ...mockProps,
      isCompleted: true,
      explanation: 'This is a very detailed explanation that goes into the specifics of how CSS centering works, including browser compatibility concerns and alternative methods.'
    }
  ...
  it('should handle rapid open/close cycles', () => {
    ...
    // Rapidly toggle
    rerender(<ThemeProvider><HintPopup {...{ ...mockProps, isOpen: false }} /></ThemeProvider>)
    rerender(<ThemeProvider><HintPopup {...{ ...mockProps, isOpen: true }} /></ThemeProvider>)
    rerender(<ThemeProvider><HintPopup {...{ ...mockProps, isOpen: false }} /></ThemeProvider>)
